...
GPU_IDS="${GPU_IDS:-0,1}"
CONFIG_PATH="${CONFIG_PATH:-${PROJECT_DIR}/configs/seg/train_random_100epochs.yaml}"
SESSION_NAME="${SESSION_NAME:-seg-random-100epochs}"
...
tmux new-session -d -s "${SESSION_NAME}" \
  "cd \"${PROJECT_DIR}\" && CUDA_VISIBLE_DEVICES=\"${GPU_IDS}\" \"${PYTHON}\" src/train_segmentation.py --config \"${CONFIG_PATH}\" --random-init --gpus \"${LOCAL_GPU_IDS}\""
